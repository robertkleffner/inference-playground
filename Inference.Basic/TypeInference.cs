...
namespace Inference.Basic
{
    ...
    public class TypeInference
    {
        ...
        private TypeInference AddIntro(string name) =>
            new TypeInference(this._state.Push(new TypeVariableIntro(name)));
        ...
        private TypeInference SkimContext(out IImmutableList<(string name, IType? definition)> skimmed)
        {
            skimmed = ImmutableList<(string name, IType? definition)>.Empty;
            ...
            while (!(last is LocalityMarker))
            {
                skimmed = last switch
                {
                    TypeVariableIntro intro => skimmed.Add((intro.Name, null)),
                    TypeVariableDefinition def => skimmed.Add((def.Name, def.Definition)),
        ...
        private TypeScheme MakeScheme(IReadOnlyList<(string name, IType? definition)> skimmedFromContext, IType ungeneralized)
        {
            var generalized = ungeneralized;
            var quantified = new List<string>();
            foreach (var (typeVarName, typeVarDef) in skimmedFromContext)
            {
                if (typeVarDef != null)
                {
